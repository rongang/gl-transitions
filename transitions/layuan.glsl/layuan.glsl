...
vec3 crossFade(in vec2 uv,in vec2 uv2, in float dissolve) {
      // 定义缩放的参数。进度控制视频的缩放程度
    float zoomFrom = mix(1.0, 0.9, Exponential_easeInOut(0.0, 1.0, 0.8, progress)); // 第一段视频从 1x 缩小到 2x
    float zoomTo = mix(2.0, 1.0,  Exponential_easeInOut(0.0, 1.0, 0.6, progress));   // 第二段视频从 0.5x 放大到 1x
    return mix(getFromColor(zoom(uv2, zoomFrom)).rgb, getToColor(zoom(uv, zoomTo)).rgb, dissolve);
}
